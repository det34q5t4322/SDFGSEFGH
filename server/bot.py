...
def get_webapp_url(group: Optional[str] = None) -> Optional[str]:
    ...
    sep = "&" if "?" in base else "?"
    if group:
        return f"{base}{sep}v=20260917_secret_v1&group={urllib.parse.quote(group)}"
    return f"{base}{sep}v=20260917_secret_v1"
...
async def post_init(application) -> None:
    ...
    try:
        ...
        if WEB_APP_URL:
            wa_menu_url = WEB_APP_URL.rstrip("/") + "/?v=20260917_secret_v1"
