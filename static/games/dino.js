/**
 * Google Chrome Dino Runner (Original Chromium Engine)
 * Pure vanilla ES module for College Schedule WebApp
 * 100% Authentic Chromium offline game with official spritesheets,
 * sound effects, physics, night mode, obstacle generators, and touch controls.
 *
...
export function mount(container, options = {}) {
  ...
  const bestScore = parseInt(localStorage.getItem('game_dino_best') || '0', 10);
  ...
  container.innerHTML = `
    <div class="dino-game-wrap" id="dinoWrap">
      <div class="game-hud">
        <div class="game-hud-scores">
          <div class="game-hud-box">
            ...
            <span class="game-hud-value" id="dinoScore">0</span>
          </div>
          <div class="game-hud-box">
        ...
        <div class="game-hud-controls">
          <button class="game-ctrl-btn" id="dinoPauseBtn" type="button" aria-label="Пауза" title="Пауза">
          ...
          <button class="game-ctrl-btn" id="dinoResetBtn" type="button" aria-label="Заново" title="Заново">
      ...
      <div class="dino-stage-container" id="dinoStage"></div>

      <!-- Tactile on-screen buttons for mobile -->
      <div class="dino-mobile-controls">
        <button class="dino-ctrl-btn dino-btn-jump" id="dinoJumpBtn" type="button">
          <svg class="lucide-icon" viewBox="0 0 24 24" width="22" height="22"><polygon points="12 4 4 17 20 17 12 4"/></svg>
          <span>Прыжок</span>
        </button>
        <button class="dino-ctrl-btn dino-btn-duck" id="dinoDuckBtn" type="button">
          <svg class="lucide-icon" viewBox="0 0 24 24" width="22" height="22"><polygon points="12 20 4 7 20 7 12 20"/></svg>
          <span>Пригнуться</span>
        </button>
      </div>

      <!-- Hidden Assets for Runner sprite loading -->
      <div style="display:none;" id="dinoHiddenAssets">
        <img id="offline-resources-1x" src="/static/games/assets/dino/100-offline-sprite.png" alt="sprites 1x">
        <img id="offline-resources-2x" src="/static/games/assets/dino/200-offline-sprite.png" alt="sprites 2x">
      </div>
    </div>
  `;
  ...
  const runner = new Runner('#dinoStage', {
    onScore: (actualDist, isBest) => {
      ...
      if (scoreEl) scoreEl.textContent = actualDist;
      if (isBest && bestEl) bestEl.textContent = actualDist;
      if (typeof onScoreUpdate === 'function') {
        onScoreUpdate(actualDist, parseInt(localStorage.getItem('game_dino_best') || '0', 10));
      }
    },
    onCrash: (finalDist) => {
      triggerHaptic('error');
      if (typeof onGameOver === 'function') {
        onGameOver(finalDist, false);
      }
    }
  });

  // Connect HUD buttons
  const pauseBtn = document.getElementById('dinoPauseBtn');
  if (pauseBtn) {
  ...
  const resetBtn = document.getElementById('dinoResetBtn');
  if (resetBtn) {
  ...
  if (duckBtn) {
    ...
    duckBtn.addEventListener('pointercancel', endDuck);
  }
...
function Runner(outerContainerId, opt_config) {
    ...
    Runner.prototype = {
        /**
         * Cleanly unmount and teardown runner without leaks.
         */
        destroy: function () {
            this.stop();
            this.stopListening();
            if (this.boundVisibilityChange) {
                document.removeEventListener(Runner.events.VISIBILITY, this.boundVisibilityChange);
                window.removeEventListener(Runner.events.BLUR, this.boundVisibilityChange);
                window.removeEventListener(Runner.events.FOCUS, this.boundVisibilityChange);
            }
            if (this.boundResize) {
            ...
            if (this.outerContainerEl) {
                this.outerContainerEl.classList.remove(Runner.classes.INVERTED);
        ...
        loadSounds: function () {
            ...
            try {
                ...
                var soundMap = {
                    BUTTON_PRESS: SOUND_PRESS_SRC,
                    HIT: SOUND_HIT_SRC,
                    SCORE: SOUND_SCORE_SRC
                };

                for (var sound in soundMap) {
                    var soundSrc = soundMap[sound];
        ...
        update: function () {
            ...
            if (this.playing) {
                ...
                if (!collision) {
                    ...
                    var curActual = this.distanceMeter.getActualDistance(Math.ceil(this.distanceRan));
                    if (this.config.onScore) {
                        this.config.onScore(curActual, curActual > this.highestScore);
                    }
        ...
        gameOver: function () {
            ...
            if (actualDistance > this.highestScore) {
                this.highestScore = actualDistance;
                this.distanceMeter.setHighScore(this.highestScore);
                try {
                    localStorage.setItem('game_dino_best', String(this.highestScore));
                } catch (_) {}
            }
            if (this.config.onCrash) {
                this.config.onCrash(actualDistance);
            }
         ...
         */
        setArcadeMode() {
            // Embedded inside schedule app modal
        },
        ...
        invert: function (reset) {
            var target = this.outerContainerEl || document.getElementById('dinoStage');
            if (!target) return;
            if (reset) {
                target.classList.toggle(Runner.classes.INVERTED, false);
                this.invertTimer = 0;
                this.inverted = false;
            } else {
                this.inverted = target.classList.toggle(Runner.classes.INVERTED, this.invertTrigger);
            }
        }
    };
